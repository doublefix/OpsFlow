...
func main() {
	...
	if err != nil {
		fmt.Printf("无法加载 kubeconfig: %v", err)
	}

	clientset, err := kubernetes.NewForConfig(cfg)
	if err != nil {
		fmt.Printf("无法创建 Kubernetes 客户端: %v", err)
	}
	...
	if wsErr != nil || spdyErr != nil {
		fmt.Printf("初始化执行器失败: websocket: %v, spdy: %v", wsErr, spdyErr)
	}
	...
	if err != nil {
		fmt.Printf("创建 FallbackExecutor 失败: %v", err)
	}
